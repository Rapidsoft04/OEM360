...
public interface DepartmentApproverRepository extends JpaRepository<DepartmentApprover, Long> {
	...
	@Query(value = "SELECT * FROM department_approver where app_owner_id=?1 or agm_id=?1 or dgm_id=?1", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM department_approver where agm_id=?1 or dgm_id=?1", nativeQuery = true)
	...
	Optional<DepartmentApprover> findAgmOrApplicationOwnerByUserId(Long userId);
	
	@Query(value = "SELECT * FROM department_approver where department_id=?1", nativeQuery = true)
	List<DepartmentApprover> findDepartmentApproversByDepartmentId(Long departmentId);
