...
public class MisReportServiceImpl implements MisReportService {
	...
	@Override
	public Response<?> exportMisReportDataV2(SearchDto searchDto) {
		try {
			String fromDate = searchDto.getFromDate();
			String toDate = searchDto.getToDate();
			String addedFromTime = "00:00:00";
					...
					if (list != null && list.size() > 0) {
						...
						for (String str : departmentName) {
							...
							for (RecommendationResponseDto dto : list) {
								...
								if (dto.getDepartment().getName().equals(str) && dto.getStatus().getId()
...
//									System.out.println(recommendation.getReferenceId());
								...
								if (dto.getDepartment().getName().equals(str)
										&& dto.getStatus().getId().longValue() >= StatusEnum.Approved.getId()
												.longValue()
										&& dto.getStatus().getId().longValue() < StatusEnum.Released.getId()
												.longValue()) {
...
//							List<RecommendationResponseDto> totalRecommendationForThisDept = new ArrayList<>();
							if (i == 1) {
								rowNum = 0;
								Sheet sheet1 = workbook.createSheet("Completed");
								...
								rowNum = rowNum + 1;
								for (String str : departmentName) {
									List<RecommendationResponseDto> completedRecommendationForThisDept = list.stream()
											.filter(e -> e.getDepartment().getName().equals(str) && e.getStatus()
...
//							for (String str : departmentName) {
//
//								List<RecommendationResponseDto> pendingApprovalListForThisDept = list.stream()
